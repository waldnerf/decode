...
class X_dn_cmtsk(HybridBlock):
    def __init__(self, nfilters_init, depth, NClasses,widths=[1], psp_depth=4,verbose=True, norm_type='BatchNorm', norm_groups=None,nheads_start=8, upFuse=False, ftdepth=5,**kwards):
        ...
        with self.name_scope():
            ...
            self.features = X_dn_features(nfilters_init=nfilters_init, depth=depth, widths=widths, psp_depth=psp_depth, verbose=verbose, norm_type=norm_type, norm_groups=norm_groups,  nheads_start=nheads_start,  upFuse=upFuse,  ftdepth=ftdepth, **kwards)
